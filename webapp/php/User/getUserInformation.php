...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        $sql2 = "SELECT id, name FROM interest_category INNER JOIN rel_user_int_cat WHERE user_email='".$row['email']."' AND rel_user_int_cat.int_cat_id = interest_category.id";
